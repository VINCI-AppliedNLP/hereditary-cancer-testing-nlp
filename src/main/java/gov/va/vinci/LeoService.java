...
public class LeoService {
    ...
    public static void main(String[] args) {
        LeoService current_service = new LeoService();

        if (args.length == 0) {
            current_service.serviceConfigFile = new File[]{new File("config/ServiceConfig.groovy")};
            current_service.pipeline = new String[]{"gov.va.vinci.pipeline.Biomarker_Pipeline"};
    ...
    public static void printUsage() {
        CmdLineParser parser = new CmdLineParser(new LeoService());
        System.out.print("Usage: java " + LeoService.class.getCanonicalName());
